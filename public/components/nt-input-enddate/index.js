  ...
  methods: {
    enddateRule(rule, value, callback) {
      ...
      if (!regExp.test(value) && value) {
        ...
        return callback(false, "结束日期不得早于开始日期");
      } else if (begindate == enddate && this.data.isEqual && value) {
        return callback(false, "结束日期不能等于开始日期");
    ...
    handleInput(e) {
      if (e) {
        ...
        this.$nextTick(() => {
         ...
         let groupId = this.data.group;
        let group = manageDateGroups(groupId, "update", "enddate", v);
      }
    },
  },
  ...
  observers: {
    modelValue: {
      handler(val) {
        ...
        let groupId = this.data.group;
        let group = manageDateGroups(groupId, "update", "enddate", val);
